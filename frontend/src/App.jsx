  ...
  return (
    <div className="relative">
      ...
      <div className="rounded-2xl overflow-hidden border border-gray-200"
        style={{ boxShadow: "0 8px 48px rgba(109,40,217,0.14), 0 2px 8px rgba(0,0,0,0.06)" }}>
        ...
        <div className="bg-gray-50 border-b border-gray-200 px-4 py-2.5 flex items-center gap-3">
          ...
          <div className="flex-1 bg-white border border-gray-200 rounded-md px-3 py-1 flex items-center gap-2">
            <svg viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth={2} className="w-3 h-3 flex-shrink-0">
          ...
          <div className="flex bg-white" style={{ minHeight: 360 }}>
              ...
              <div className="space-y-0.5">
                {categories.map((cat) => (
                  <button key={cat.name} onClick={() => setActiveCategory(cat.name)}
                    className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-[10px] font-semibold transition-all ${
                      activeCategory === cat.name ? "bg-violet-600 text-white" : "text-gray-600 hover:bg-gray-100"
                    }`}>
                    <span className="flex items-center gap-1">
                      <span className="text-[9px]">{cat.icon}</span>{cat.name}
                    </span>
                    <span className={`text-[9px] ${activeCategory === cat.name ? "text-violet-200" : "text-gray-400"}`}>
                      {cat.count}
                    </span>
                  </button>
                ))}
                <button className="w-full flex items-center gap-1 px-2 py-1.5 rounded-lg text-[10px] text-violet-600 font-bold hover:bg-violet-50 mt-1.5">
                  + Tambah Tipe
                </button>
              </div>
            </div>
            <div className="flex-1 p-4 overflow-hidden">
              <div className="flex items-center justify-between mb-3">
                ...
                <button className="flex items-center gap-1 bg-violet-600 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg">
                  + Tambah Prompt
                </button>
              </div>
              <div className="space-y-2.5">
                {prompts.map((p, i) => (
                  <div key={i} className="p-3 rounded-xl border border-gray-100 bg-white cursor-pointer hover:border-violet-200 hover:shadow-md transition-all"
                    style={{ boxShadow: "0 1px 8px rgba(109,40,217,0.06)" }}>
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <p className="text-gray-900 text-xs font-bold leading-tight">{p.title}</p>
                      <svg viewBox="0 0 24 24"
                        className={`w-3.5 h-3.5 flex-shrink-0 ${p.starred ? "fill-amber-400 stroke-amber-400" : "fill-none stroke-gray-300"}`}
                        strokeWidth={2}>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex gap-1">
                        {p.tags.map((t) => (
                          <span key={t} className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${tagColors[t] || "bg-gray-100 text-gray-600"}`}>{t}</span>
                        ))}
                      </div>
          ...
          <div className="bg-white p-6" style={{ minHeight: 360 }}>
            <div className="mb-4">
              <div className="flex items-center gap-3 bg-violet-50 border-2 border-violet-300 rounded-xl px-4 py-3 mb-2">
                <svg viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth={2.5} className="w-5 h-5 flex-shrink-0">
                  <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
                </svg>
            ...
            <div className="space-y-2">
              {prompts.slice(0, 2).map((p, i) => (
                <div key={i} className="p-3 rounded-xl border border-gray-100 bg-white hover:border-violet-200 transition-all cursor-pointer"
                  style={{ boxShadow: "0 1px 8px rgba(109,40,217,0.06)" }}>
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className="text-gray-900 text-xs font-bold leading-tight">{p.title}</p>
                    <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 flex-shrink-0 fill-amber-400 stroke-amber-400" strokeWidth={2}>
                      <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                    </svg>
                  </div>
          ...
          <div className="bg-white p-6" style={{ minHeight: 360 }}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="border-2 border-violet-300 rounded-xl px-3 py-2.5 bg-violet-50/40">
                  <p className="text-gray-900 text-xs font-semibold">Buat thread Twitter tentang tips produktivitas</p>
                </div>
              </div>
              <div>
                ...
                <div className="border border-gray-200 rounded-xl px-3 py-2.5 bg-gray-50/60" style={{ minHeight: 80 }}>
                  <p className="text-gray-600 text-[10px] leading-relaxed">Buatkan 10 tweet thread yang engaging tentang tips produktivitas untuk developer...</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {["Twitter", "Konten"].map((t) => (
                      <span key={t} className="text-[9px] font-bold px-2 py-1 rounded-full bg-pink-100 text-pink-700">{t} ×</span>
                    ))}
...
export default function App() {
          ...
          <div className="modal-in relative bg-white rounded-3xl w-full max-w-sm overflow-hidden"
            ...
            <div className="px-8 py-8 text-center">
              ...
              <div className="flex flex-col gap-3">
                ...
                <button onClick={() => setShowLoginModal(false)}
                  className="text-gray-400 hover:text-gray-600 text-sm font-medium mt-1 transition-colors">
                  Kembali ke Beranda
                </button>
              </div>
            </div>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            <a href="#beranda" className="text-zinc-900 font-bold text-sm hover:border-b-2 hover:border-violet-500 pb-0.5 transition-all duration-300">Beranda</a>
            <a href="#about"   className="text-zinc-900 font-bold text-sm hover:border-b-2 hover:border-violet-500 pb-0.5 transition-all duration-300">About</a>
            <a href="#info"    className="text-zinc-900 font-bold text-sm hover:border-b-2 hover:border-violet-500 pb-0.5 transition-all duration-300">Info</a>
            <a href="/Fituroption" onClick={handlePromptLabClick}
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleLogout}
                  className="text-sm font-semibold text-gray-700 hover:text-violet-700 px-4 py-2 rounded-lg hover:bg-violet-50 transition-all border border-gray-200">
                  Logout
                </button>
              </div>
            ) : (
              <>
                ...
                <a href="/Register"
                  className="text-sm font-bold text-white bg-violet-600 hover:bg-violet-700 px-5 py-2.5 rounded-xl btn-glow transition-all">
                  Register
                </a>
              </>
            )}
  ...
  className="md:hidden flex flex-col justify-center gap-[5px] w-9 h-9 rounded-lg hover:bg-violet-50 transition-colors"
  aria-label="Toggle menu"
>
  <i className="ri-menu-line text-zinc-900 text-2xl"></i>
</button>
        </div>
      </nav>
      ...
      <div className="fixed inset-0 z-40 md:hidden" style={{ pointerEvents: menuOpen ? "auto" : "none" }}>
        ...
        <div style={{
          ...
          <div className="px-5 py-4 flex flex-col gap-1">
            ...
            {/* User info */}
            {isLoggedIn && (
              <div className="flex items-center gap-3 bg-violet-50 rounded-xl p-3 mb-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400">Sudah login</p>
                </div>
              </div>
            )}
            ...
            {/* Nav links */}
            {[
              { href: "#beranda", icon: "ti-home",        label: "Beranda" },
              { href: "#about",   icon: "ti-user",        label: "About"   },
              { href: "#info",    icon: "ti-info-circle", label: "Info"    },
            ].map(({ href, icon, label }) => (
              <a key={label} href={href} onClick={() => setMenuOpen(false)}
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 font-semibold hover:bg-violet-50 hover:text-violet-700 transition-colors">
                <i className={`ti ${icon} text-gray-400 text-base`} />
                {label}
              </a>
            ))}

            <a href="/Fituroption" onClick={(e) => { setMenuOpen(false); handlePromptLabClick(e); }}
              className="flex items-center gap-3 px-4 py-3 rounded-xl text-violet-700 font-bold bg-violet-50 hover:bg-violet-100 transition-colors">
              <i className="ti ti-sparkles text-violet-500 text-base" />
              PromptLab
            </a>

            {/* Auth */}
            <div className="border-t border-gray-100 pt-3 mt-1 flex flex-col gap-2 pb-2">
              {isLoggedIn ? (
                <button onClick={() => { setMenuOpen(false); handleLogout(); }}
                  className="flex items-center gap-3 px-4 py-3 rounded-xl text-red-500 font-semibold hover:bg-red-50 transition-colors w-full text-left">
                ...
                <>
                  <a href="/Login" onClick={() => setMenuOpen(false)}
                    className="flex items-center justify-center px-4 py-3 rounded-xl border border-gray-200 text-gray-700 font-semibold hover:bg-gray-50 transition-colors text-sm">
                    Login
                  </a>
                  <a href="/Register" onClick={() => setMenuOpen(false)}
                    className="flex items-center justify-center px-4 py-3 rounded-xl bg-violet-600 text-white font-bold hover:bg-violet-700 transition-colors text-sm btn-glow">
                    Daftar Gratis
                  </a>
                </>
              )}
  ...
  {/* Teks + bintang dijadikan satu grup */}
    ...
    <div className="flex">
      {[...Array(5)].map((_, i) => (
        <span key={i} className="text-amber-400 text-sm">★</span>
            ...
            <div className="flex-1 relative w-full">
                ...
                <div className="bg-white rounded-2xl card-shadow border border-gray-100 overflow-hidden w-full max-w-md mx-auto">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-1.5 flex-1 mx-3 border border-gray-200">
                      <svg viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth={2} className="w-3.5 h-3.5 flex-shrink-0">
                        <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
                      </svg>
                        ...
                        {categories.map((cat) => (
                          <button key={cat.name} onClick={() => setActiveCategory(cat.name)}
                            className={`w-full flex items-center justify-between px-2 py-1.5 rounded-lg text-xs font-medium transition-all ${
                              activeCategory === cat.name ? "bg-violet-600 text-white" : "text-gray-600 hover:bg-gray-50"
                            }`}>
                            <span className="flex items-center gap-1.5">
                              <span className="text-[10px]">{cat.icon}</span>{cat.name}
                            </span>
                            <span className={`text-[10px] font-semibold ${activeCategory === cat.name ? "text-violet-200" : "text-gray-400"}`}>
                              {cat.count}
                            </span>
                          </button>
                        ))}
                        <button className="w-full flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-xs text-violet-600 font-semibold hover:bg-violet-50 mt-2">
                          + Tambah Tipe
                        </button>
                      </div>
                    </div>
                        ...
                        {prompts.map((p, i) => (
                          <div key={i} className="p-2.5 rounded-xl border border-gray-100 bg-white card-shadow cursor-pointer hover:border-violet-200 transition-all">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex gap-1 flex-wrap">
                                {p.tags.map((t) => (
                                  <span key={t} className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${tagColors[t] || "bg-gray-100 text-gray-600"}`}>{t}</span>
                                ))}
                              </div>
                              <span className="text-gray-300 text-[9px]">{p.date}</span>
                            </div>
              ...
              <div className="float-anim-2 absolute -top-4 -right-4 w-14 h-14 bg-white rounded-2xl card-shadow flex items-center justify-center z-20">
              ...
              <div className="float-anim absolute bottom-8 -left-6 w-12 h-12 bg-violet-600 rounded-2xl flex items-center justify-center z-20 shadow-lg">
                <svg viewBox="0 0 24 24" fill="white" className="w-5 h-5">
                  ...
                  <path d="m21 21-4.35-4.35" stroke="white" strokeWidth="2" fill="none" />
                </svg>
              </div>
              <div className="absolute -bottom-4 right-10 w-10 h-10 bg-amber-400 rounded-full flex items-center justify-center z-20 shadow-lg float-anim-2">
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <AnimatedSection className="flex-1 w-full">
              <img src="/assets/ai.jpeg" alt="image-description" className="rounded-2xl float-anim w-full" />
            </AnimatedSection>
            <div className="flex-1">
              <AnimatedSection>
                  ...
                  {["Simpan tanpa batas", "Temukan dengan mudah", "Tingkatkan produktivitas harian"].map((item) => (
                    <div key={item} className="flex items-center gap-3">
                      <div className="w-5 h-5 rounded-full bg-violet-600 flex items-center justify-center flex-shrink-0">
                        <svg viewBox="0 0 24 24" fill="white" className="w-3 h-3">
                          <path d="M20 6L9 17l-5-5" stroke="white" strokeWidth="2.5" fill="none" />
                        </svg>
                      </div>
      ...
      <section className="py-24 bg-white" id="info">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <AnimatedSection delay={0.1} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-14">

            {/* SaaS */}
            <div className="relative overflow-hidden rounded-2xl cursor-pointer h-64 border-2 border-transparent
              hover:border-fuchsia-500 hover:scale-[1.03] hover:shadow-[0_0_30px_rgba(192,38,211,0.45)]
              transition-all duration-300 group">
              <img src="/assets/saas.jpg" alt="Nexora Dashboard" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" />
              <div className="absolute top-3 left-3 bg-fuchsia-500 text-white text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md">SaaS Platform</div>
            ...
            <div className="relative overflow-hidden rounded-2xl cursor-pointer h-64 border-2 border-transparent
              ...
              transition-all duration-300 group">
              <img src="/assets/pos-cs.png" alt="CaféPOS Pro" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" />
              <div className="absolute top-3 left-3 bg-amber-400 text-amber-900 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md">POS Cashier</div>
              <div className="absolute bottom-0 left-0 right-0 p-4">
            ...
            <div className="relative overflow-hidden rounded-2xl cursor-pointer h-64 border-2 border-transparent
              ...
              transition-all duration-300 group">
              <img src="/assets/absensi.png" alt="Attendly School" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" />
              <div className="absolute top-3 left-3 bg-cyan-400 text-cyan-900 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md">Sistem Absensi</div>
              <div className="absolute bottom-0 left-0 right-0 p-4">
            ...
            <div className="relative overflow-hidden rounded-2xl cursor-pointer h-64 border-2 border-transparent
              ...
              transition-all duration-300 group">
              <img src="/assets/kosan.png" alt="KosanKu Finder" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/10 to-transparent" />
              <div className="absolute top-3 left-3 bg-emerald-400 text-emerald-900 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md">Landing Page</div>
              <div className="absolute bottom-0 left-0 right-0 p-4">
      ...
      {/* ── FOOTER ── */}
      <footer className="relative overflow-hidden" style={{ background: "#6d28d9", padding: "40px 24px 0" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  ...
                  <a key={title} href={href || "#"} onClick={onClick}
                    style={{
                      display: "flex", alignItems: "center", gap: 8,
                      background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.11)",
                      borderRadius: 8, padding: "7px 10px", textDecoration: "none", transition: "all .15s",
                    }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {[
                  ...
                  <a key={label} href={href || "#"} onClick={onClick}
                    ...
                    onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "transparent"; e.currentTarget.style.color = "rgba(255,255,255,0.78)"; }}
                  >
